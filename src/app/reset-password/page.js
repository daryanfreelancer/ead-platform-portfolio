...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (accessToken && refreshToken && type === 'recovery') {
          ...
          if (error) {
            ...
            // Limpar hash da URL para evitar reprocessamento
    ...
    // Escutar apenas eventos específicos relevantes
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      // Apenas processar se ainda não foi processado e é um evento de recuperação
      if (!processed && event === 'PASSWORD_RECOVERY' && session) {
        setSessionReady(true)
      }
    })
